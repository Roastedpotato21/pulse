...
def _release_files(directory: Path) -> None:
    wheel = directory / "pulse_coding_agent-0.1.1-py3-none-any.whl"
    with zipfile.ZipFile(wheel, mode="w") as archive:
        archive.writestr(
            "pulse_coding_agent-0.1.1.dist-info/METADATA",
        ...
        archive.writestr(
            "pulse_coding_agent-0.1.1.dist-info/entry_points.txt",
    ...
    with tarfile.open(
        directory / "pulse_coding_agent-0.1.1.tar.gz", mode="w:gz"
    ) as archive:
        readme = directory / "README.md"
        readme.write_text("Pulse", encoding="utf-8")
        archive.add(readme, arcname="pulse_coding_agent-0.1.1/README.md")
        readme.unlink()
...
def test_release_metadata_binds_hashes_to_source_commit(tmp_path: Path) -> None:
    ...
    assert "pulse_coding_agent-0.1.1-py3-none-any.whl" in checksums
    assert "pulse_coding_agent-0.1.1.tar.gz" in checksums
